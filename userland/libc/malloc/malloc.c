...
uintptr_t total_heap_size = 0;

struct rng_ctx heap_rng_ctx;
...
static void *memalloc(size_t length) {
  ...
  assert(ERROR_SUCCESS ==
         sa_mmap(NULL, length, 0 /*TODO prot*/, MAP_ANONYMOUS, 0, 0, &out));
  //  assert(ERROR_SUCCESS ==
  //         sa_mmap((void *)0x41, 0x42, 0x43, 0x44, 0x45, 0x46, (void *)0x47));
  return out;
}

int init_heap(void) {
  malloc_head = (MallocHeader *)memalloc(NEW_ALLOC_SIZE);
...
MallocHeader *find_free_entry(uintptr_t s) {
...
void *int_malloc(size_t s, int recursion) {
  ...
  sarandom_buf(&heap_rng_ctx, rc, s);
  return rc;
}
...
void free(void *p) {
  ...
  sarandom_buf(&heap_rng_ctx, p, h->size);
