...
int64_t sv_parse_number(struct sv input, struct sv *rest, int *got_num) {
  return sv_parse_unsigned_number(input, rest, got_num);
}
...
// FIXME: DOES NOT HANDLE SIGNED NUMBERS
...
struct sv sv_split(const struct sv input, struct sv *rest, struct sv delim) {
  ...
  size_t n = input.length - p.length;
  return sv_take(input, NULL, n);
}
