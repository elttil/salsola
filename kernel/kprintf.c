...
int print_unum(struct print_context *ctx, unsigned long long n, int base,
               char *char_set, int prefix, int zero_padding,
               int right_padding) {
  int c = 0;
  char str[32];
  int i = 0;
...
int print_uhex(struct print_context *ctx, unsigned long long n, int prefix,
               int zero_padding, int right_padding) {
  return print_unum(ctx, n, 16, "0123456789abcdef", prefix, zero_padding,
                    right_padding);
}
...
int vkcprintf(struct print_context *ctx, const char *fmt, va_list ap) {
  ...
  for (; *s; s++) {
    ...
    case 'u':
      assert(-1 == precision);
      if (2 == long_level) {
        rc += print_int(ctx, va_arg(ap, unsigned long long), prefix,
                        zero_padding, right_padding);
      } else if (1 == long_level) {
        rc += print_int(ctx, va_arg(ap, unsigned long long), prefix,
                        zero_padding, right_padding);
      } else {
    ...
    case 'x':
      assert(-1 == precision);
      rc += print_uhex(ctx, va_arg(ap, const u64), prefix, zero_padding,
                       right_padding);
      cont = 0;
      break;
    case 'o':
